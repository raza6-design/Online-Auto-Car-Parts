...
if(isset($_GET['st']))
{
	...
	if(mysqli_affected_rows($con) == 1)
	{
		echo "<script>alert('Delivery status Updated successfully...');</script>";
	 	...
	 	$sqlbilling  = "SELECT * FROM billing LEFT JOIN cutomer ON billing.customerid=cutomer.customer_id LEFT JOIN purchase ON purchase.billingid=billing.billingid LEFT JOIN product ON  product.productid=purchase.productid WHERE billing.billingid='$_GET[billno]'";
...
<tr>
<h2 class="mh2"> <span style=color:white;>Auto</span><span  style=color:yellow;>_Car_parts</span><span style=color:white;>_Store</span></h2>
	
</tr>
<tr>
	<td colspan="3"><center>Raza,Punjab</center></td>
</tr>

<tr>
	<td colspan="2"><b>Customer:</b>  <?php echo $rsbilling['customername']; ?></td>
	<td><b>Date:</b>  <?php echo $rsbilling['purchasedate']; ?> </td>
</tr>

<tr>
	<td colspan="2"><b>Address:</b> <?php echo $rsbilling[4]; ?>,<?php echo $rsbilling[5]; ?>,<br>State: <?php echo $rsbilling[6]; ?>,<br>PIN: <?php echo $rsbilling[8]; ?> </td>
	<td><b>Bill No:</b> <?php echo $rsbilling['billingid']; ?></td>
</tr>

<tr>
	<td colspan="2"><b>Contact No:</b><?php echo $rsbilling[7]; ?> </td>
	<td></td>
</tr>
</table>
...
<table   class="table table-striped table-bordered dataTable no-footer" cellspacing="0" width="100%" role="grid" aria-describedby="example_info" style="width: 100%;background-color:white;">
	...
	<?php
	$subtotal1=0;
	$sql  = "SELECT * FROM purchase LEFT JOIN product ON purchase.productid=product.productid LEFT JOIN billing ON purchase.billingid=billing.billingid WHERE purchase.billingid='$_GET[billno]'";
...
else
{
	...
	echo "<b>Offer - </b>$offeramt% off";
		...
		echo "</td>
			<td>Rs.  $totamt</td>
			<td>$rs[qty]</td>
			<td  align='right'>Rs. " .  $totamt * $rs['qty'] . "</td>
...
<tr>
	<th colspan="2">CGST : (<?php echo $rsbilling['cgsttax']; ?>% )</th>
	<td align="right">RS <?php  $cgst =  $subtotal * $rsbilling['cgsttax']/100; 
	
	echo number_format($cgst,2);
	?></td>
</tr>
<tr>
	<th colspan="2">SGST : (<?php echo $rsbilling['sgsttax']; ?>%)</th>
	<td align="right">RS <?php  $sgst = $subtotal * $rsbilling['sgsttax']/100; 
...
?>
<tr>
	<th colspan="2">IGST:(<?php echo $rsbilling['igsttax']; ?>% )</th>
	<td align="right"> RS <?php  $igst =  $subtotal * $rsbilling['igsttax']/100; 
...
<tr>
	...
	<td align="right">RS <?php echo $total  =  $subtotal + $cgst + $sgst + $igst; ?></td>
</tr>
 <tr>
	<th colspan="2">Grand total: </th>
	<td align="right">RS <?php echo $grandtotal =  $total - $discamt ; ?></td>
</tr>
...
<tr>
	...
	<th>
	<?php echo $rsbilling['deliverystatus']; ?>
	</th>
	<th>
